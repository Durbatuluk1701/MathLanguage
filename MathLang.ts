...
const tokens = Tokenize(testStr, TDL);
console.log(tokens);

const gram: Grammar<(n: number) => number> = [
  {
    type: "Rule",
    name: "BinaryOp",
    pattern: [["PLUS"], ["CARET"], ["STAR"], ["MINUS"]],
    callback: (
      r: RuleMatch<(n: number) => number>,
      context: { stack: ((n: number) => number)[] }
    ) => {
      if (r.match[0].rule.type !== "Token")
        throw new Error("Only expected tokens as a binary operator");

      const lastVal = context.stack.pop();
      if (!lastVal) throw new Error("Cannot do operation with empty stack");
  ...
  },
  {
    ...
    callback(
      ...
      if (r.rule.type !== "Rule") throw new Error("This should be a rule!");
      const match0 = r.match[0];

      if (match0.rule.name === "EMPTY") {
        const currentTop = context.stack.pop();
        if (currentTop) return currentTop;
        ...
        switch (match0.rule.name) {
          case "NUM":
            ...
            if (stackTop) {
              ...
              context.stack.push((n: number) => stackTop(numberOutput));
            } else {
            ...
            if (match1 && match1.match.length > 0) {
              ...
              if (match1.rule.type !== "Rule")
                throw new Error(
                  "Expecting a 'NonRecPhrase', but we did not encounter one"
                );
          ...
          case "LPAREN":
            ...
            if (match1.rule.type !== "Rule")
              throw new Error(
                "Expecting a 'Phrase', but we did not encounter one"
              );
            ...
            if (previousStackTop) {
              // We have something at the top of the stack to work with
              context.stack.push((n: number) =>
                previousStackTop(internalOutput(n))
              );
            } else {
              // Nothing at the top of the stack, so just add first item
          ...
          default:
            throw new Error(
              `Expecting either a NUM or LPAREN, but instead received a ${match0.rule.name}`
            );
        }
...
if (!phraseRule) throw new Error("COULDNT FIND PHRASE RULE");

const trimmedWhitespace = tokens.filter((val) => val.name !== "WHITESPACE");

// console.log(trimmedWhitespace);

const parseOut = Parser(3, trimmedWhitespace, gram, phraseRule);
